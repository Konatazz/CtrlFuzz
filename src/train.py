...
@click.option("--data_path", "data_path", type=str, default="", help="Path to the ImageNet 64x64 dataset", required=False)
...
# Model Parameters
@click.option("--embCh", "embCh", type=int, default=128, help="Note, this is scaled by 2^(chMult*layer) each U-net layer", required=False)
@click.option("--chMult", "chMult", type=int, default=1, help="Each layer has embCh*2^(chMult*layer) channels", required=False)
@click.option("--num_layers", "num_blocks", type=int, default=3, help="A value of 3 has a depth of 3, meaning there are 3 down layers and 3 up layers in the U-net", required=False)
@click.option("--blk_types", "blk_types", type=str_to_list, default="res,clsAtn,chnAtn", help="(list of \"res\", \"conv\", \"clsAtn\", \"atn\", and/or \"chnAtn\". RES: Normal residual block. CONV: ConvNext block. CLSATN: Attention block to include extra class information (Q, K are cls features). ATN: Attention ViT block for the hidden features. CLSATN: Efficient-lightweight attention block over the feature channels. EX: \"res,res,conv,clsAtn,chnAtn\"", required=False)
...
@click.option("--loadModel", "loadModel", type=bool, default=False, help="True Note that all three model files are needed for a successfull restart: the model .pkl file, the model metadata .json file, and the optimizer .pkl file.", required=False)
@click.option("--loadDir", "loadDir", type=str, default="", help="Directory of the model files to load in.", required=False)
