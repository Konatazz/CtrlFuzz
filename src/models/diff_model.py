...
from src.manifold_save import load_semantic_space
from src.manifold_save import calculate_distance_ratio, load_semantic_space
...
class diff_model(nn.Module):
    ...
    def __init__(self, inCh, embCh, chMult, num_blocks,
        ...
        self.activations = []
        self.category_centers, self.category_convex_hulls, self.lda_model, self.label_encoder, self.X_lda, self.y_encoded = load_semantic_space(
        )
    ...
    def unnoise_batch_with_interpolation(self, x_t, t_DDIM, t_DDPM, class_label=-1, w=0.0, corrected=False,
                                         target_range=(0.8, 0.9)):
        # w assertion
        assert w >= 0.0, "The value of w (classifier guidance factor) cannot be less than 0."
        ...
        if t_DDIM % 10 == 0:
            current_features = self.extract_features(x_t)
            projected_features = self.project_to_semantic_space(current_features)
            current_ratio = calculate_distance_ratio(projected_features, class_label, self.category_centers,
                                                     self.category_convex_hulls, self.X_lda, self.y_encoded)

            if not target_range[0] <= current_ratio <= target_range[1]:
                target_ratio = (target_range[0] + target_range[1]) / 2
                target_features = self.get_target_features(class_label, target_ratio)
                target_features = torch.tensor(target_features).to(x_t.device)
                target_features = target_features.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
                if target_features.size(1) == 2 and x_t.size(1) == 3:
                    target_features = torch.cat([target_features, target_features[:, :1, :, :]], dim=1)
                target_features = target_features.expand_as(x_t)
                x_t = (1 - lambda_) * x_t + lambda_ * target_features
                current_features = self.extract_features(x_t)
                projected_features = self.project_to_semantic_space(current_features)
                new_ratio = calculate_distance_ratio(projected_features, class_label,
                                                     self.category_centers, self.category_convex_hulls,
                                                     self.X_lda, self.y_encoded)
        x_0_pred = ((x_t - sqrt_1_minus_a_bar_t * noise_t) / sqrt_a_bar_t)
        if corrected:
            x_0_pred = x_0_pred.clamp(-1, 1)
        x_t_dir_pred = torch.sqrt(torch.clamp(1 - sqrt_a_bar_t1 - beta_tilde_t, 0, torch.inf)) * noise_t
        random_noise = torch.randn((noise_t.shape), device=self.device) * torch.sqrt(var_t)

        out = sqrt_a_bar_t1 * x_0_pred + x_t_dir_pred + random_noise
        if torch.any(torch.isnan(out)):
            print("Issue generating image. Image generation process generated nan values.")
            exit()
    ...
    def extract_features(self, x_t):
        ...
        with torch.no_grad():
            ...
            features = self.resnet_model(x_t).cpu().numpy().flatten()
        return features

    def project_to_semantic_space(self, features):
        projected_features = self.lda_model.transform(features.reshape(1, -1))
    ...
    def sample_imgs(self, batchSize, class_label=-1, w=0.0, save_intermediate=False, use_tqdm=False, unreduce=False,
                    corrected=False, mask=None, background=None):
        ...
        # If background and mask are provided, make sure they are in the right format
        if mask is not None and background is not None:
            mask_tensor = torch.tensor(mask).unsqueeze(0).repeat(batchSize, 3, 1, 1).to(self.device)  # [B, 3, H, W]
            background_tensor = torch.tensor(background).permute(2, 0, 1).unsqueeze(0).repeat(batchSize, 1, 1, 1).to(
                self.device)  # [B, 3, H, W]
        ...
        for t_DDIM, t_DDPM in tqdm(
                zip(reversed(range(1, num_steps + 1)), reversed(range(1, self.T + 1, self.step_size))),
                total=num_steps) if use_tqdm else zip(reversed(range(1, num_steps + 1)),
                                                      reversed(range(1, self.T + 1, self.step_size))):
            ...
            # output = self.unnoise_batch_with_interpolation(output, t_DDIM, t_DDPM, class_label, w, corrected)

            # If mask is provided, only update the masked region
            if mask is not None and background is not None:
                output = mask_tensor * output + (1 - mask_tensor) * background_tensor
